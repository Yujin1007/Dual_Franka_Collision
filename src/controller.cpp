...
void CController::read(double t, double *q, double *qdot, int check)
{
	...
	for (int i = 0; i < 7; i++)
	{
		_input[i] = (_q(i) - _min_joint_position(i)) / (abs(_max_joint_position(i) - _min_joint_position(i)) / 2) - 1;
		_input[i + 7] = (_q2(i) - _min_joint_position(i)) / (abs(_max_joint_position(i) - _min_joint_position(i)) / 2) - 1;
		if (_input[i] > 1)
		{
			_input[i] = 1;
		}
...
void CController::motionPlan()
{
	if (_bool_plan(_cnt_plan) == 1)
	{
		...
		reset_target(1.0, _q_order, 1);
		reset_target(1.0, _q_order2, 2);
...
void CController::save_stack()
{
	if (check_save_log_once == true)
	{
		...
		if (a != 0)
		{
			output.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/save_data/dual_panda/panda_0409.txt", ios::app);
			for (int i = 0; i < a; i++)
			{
				check_7++;
				if (check_7 == 2 * _k + 1)
...
void CController::load_weight()
{
	...
	////////0,7///////////////////////
	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[0].txt");
	for (int j = 0; j < _nodes0; j++)	
	{
		// cout<<endl<<endl<<"******"<<j<<"********"<<endl;
		for (int i = 0; i < 14; i++)
		{
	...
	// cout<<"\n\nweight[1][3]"<<_weight0[1][3];
	weight0.close();

	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[1].txt");
	for (int i = 0; i < _nodes0; i++)
	{
		weight0 >> _weight1[i];
	}
	weight0.close();

	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[2].txt");
	for (int j = 0; j < _nodes1; j++)
	{
		for (int i = 0; i < _nodes0; i++)
	...
	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[3].txt");
	for (int i = 0; i < _nodes1; i++)
	{
		weight0 >> _weight3[i];
	}
	weight0.close();

	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[4].txt");
	for (int j = 0; j < _nodes2; j++)
	{	
		for (int i = 0; i < _nodes1; i++)
	
		{
			weight0 >> _weight4[i][j];
		}
	}
	weight0.close();

	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[5].txt");
	for (int i = 0; i < _nodes2; i++)
	{
		weight0 >> _weight5[i];
	}
	weight0.close();

	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[6].txt");
	for (int i = 0; i < _nodes2; i++)
	{
		weight0 >> _weight6[i];
	}
	...
	weight0.open("/home/kist-robot2/Desktop/KDH/franka_panda_3/weight/dual_arm/test_heavy_0419_deep[7].txt");
	weight0 >> _weight7;
	weight0.close();

	// cout << "weight0[0][0]" << _weight0[0][0] << endl;
	// cout << "weight0[0][1]" << _weight0[0][1] << endl;
	// cout << "weight0[1][0]" << _weight0[1][0] << endl;
	// cout<<endl<<"/1/"<<_weight0[0][0];
}
...
// double CController::estimate_LR(double _input)
{
	// for(int j = 0 ; j<_nodes0; j++) // 뭐지 초기화 하는건가..
	// {
	//     _hidden0[j] = 0.0;
	//     _hidden1[j] = 0.0;
	// }
	...
	for (int j = 0; j < _nodes2; j++)
	{
		for (int i = 0; i < _nodes1; i++)
		{
			_hidden2[j] = _hidden2[j] + _hidden1[i] * _weight4[i][j];
		}
		_hidden2[j] = _hidden2[j] + _weight5[j];
		if (_hidden2[j] <= 0)
		{
			_hidden2[j] = 0.0;
		}
	...
	for (int i = 0; i < _nodes2; i++)
	{
		_output1 = _output1 + _hidden2[i] * _weight6[i];
	}
	_output1 = _output1 + _weight7;
	// cout<<"input?"<<_input[0]<<","<<_input[1]<<endl;
	// cout<<"hidden 0[9]:"<<_hidden0[9]<<"  hidden 1[1] "<<_hidden1[1]<<"  hidden 2[0]"<<_hidden2[0]<<endl;
	// cout<<"output :"<<_output1<<" weight 7: "<<_weight7<<endl;
	_output1 = 1.0 / (1.0 + exp(-_output1));
	// cout<<"output :"<<_output1<<" weight 7: "<<_weight7<<endl;
	return _output1;
}

void CController::test_LR(int check)
{
	nocheck = false;
	save_q_once = true;
	save_q_once1 = true;
	...
	if (save_q_once == true)
	{
		for (int i = 0; i < 7; i++)
		{
			if ((_q(i) <= _min_joint_position(i)) && (_q2(i) <= _min_joint_position(i)))
			{
				nocheck = true;
				break;
			}
			if ((_q(i) >= _max_joint_position(i)) && (_q2(i) >= _max_joint_position(i)))
			{
				nocheck = true;
				break;
			}
		...
		{
			clock++;
			// cout<<"collision :"<<check<<" output1 :"<<_output1;
			if (check >= 1) // 충돌 발생
			{
				_nconcnt++;
				if (_output1 < 0.5)
				{
					diff_all++;
					NconOCollX++; // 틀렸을 때
								  // cout<<"  collision(0), wrong  ";
				}
				else
				{

					// cout<<"  collision(0), correct  ";
				}
			}
			else // 충돌 안발생
			{
				...
				if (_output1 >= 0.5)
				{
					diff_all++;
					NconXCollO++; // 틀렸을 때
								  // cout<<"  collision(X), wrong  ";
				}
				else
				{

					// cout<<"  collision(X), correct  ";
				}
			}

			double tempxxx = 0.0;
			if ((diff_all + NconOCollX) != 0)
			{
				tempxxx = NconOCollX * 100;
				tempxxx = tempxxx / diff_all;
				cout << "all : " << clock << "	diff_all : " << diff_all << "		NconXCollO : " << diff_all - NconOCollX << endl;
				cout << "accuracy : " << (double)(clock - diff_all) * 100 / clock << "%		percentage : " << tempxxx << "%" << endl;

				tempxxx = _nconcnt - NconOCollX;
				tempxxx = tempxxx / _nconcnt;
				tempxxx = tempxxx * 100;
				cout << "collision O  : " << _nconcnt << " , accuracy : " << tempxxx << "%" << endl;

				tempxxx = _nconminuscnt - NconXCollO;
				tempxxx = tempxxx / _nconminuscnt;
				tempxxx = tempxxx * 100;
				cout << "collision X  : " << _nconminuscnt << " , accuracy : " << tempxxx << "%" << endl
					 << endl;
			}
			else
			{
