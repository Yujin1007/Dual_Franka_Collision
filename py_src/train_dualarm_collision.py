...
def main(dataset_path, model_path, batch_size):
    ...
    # # X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.2, random_state=2) # light datasets
    # X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.01, random_state=2) # heavy datasets
    #
    ...
    for chunk in chunks:
        ...
        X_chunk = chunk.iloc[:, :14].values
        # Y_chunk = chunk.iloc[:, 14].values.reshape(-1, 1)
        Y_chunk = chunk.iloc[:, 28].values.reshape(-1, 1)
